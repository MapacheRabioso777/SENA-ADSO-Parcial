 ...
 */
  ...
  */
  ...
  /**
   * loginUser — autentica un usuario de la tabla `users` y devuelve un JWT.
   * Body: { user, password }
   */
  async loginUser(req, res) {
    try {
      ...
      const [result] = await connect.query(
        'SELECT * FROM users WHERE User_user = ?', [user]
      );
      if (result.length === 0)
        return res.status(400).json({ error: "user not found" });

      const found = result[0];
      const valid = await comparePassword(password, found.User_password);
      if (!valid)
        return res.status(400).json({ error: "Incorrect password" });

      const token = jwt.sign(
        { id: found.User_id, role: found.Roles_fk, status: found.User_status_fk },
